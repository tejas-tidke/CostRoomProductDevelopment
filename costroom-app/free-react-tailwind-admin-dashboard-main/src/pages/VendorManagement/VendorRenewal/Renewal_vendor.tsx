...
const fetchFinalProposalTotal = async (issueKey: string): Promise<number | null> => {
  try {
    const data = await apiCall(`/api/jira/proposals/issue/${encodeURIComponent(issueKey)}`);
    if (!data) return null;
...
const Renewal_vendor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchAndEnrich = async () => {
      try {
        ...
        const data: ContractDetails[] = await apiCall('/api/jira/contracts/completed');
  ...
  useEffect(() => {
    ...
    const checkIssueStatus = async (issueKey: string): Promise<string | null> => {
      try {
        const data = await apiCall(`/api/jira/issues/${encodeURIComponent(issueKey)}`);
        const status =
    ...
    const fetchContractForIssue = async (issueKey: string): Promise<ContractDetails[] | null> => {
      try {
        const data = await apiCall(`/api/jira/contracts/byIssueKey/${encodeURIComponent(issueKey)}`);
        if (!data) return null;
        ...
        try {
          ...
          if (isTerminal(status)) {
            ...
            if (contracts && contracts.length) {
              ...
              // fallback refresh
              try {
                const data = await apiCall('/api/jira/contracts/completed');
                const mappedAll = Array.isArray(data) ? data.map(mapContractToRenewalItem) : [];
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100">
        ...
        <div className="border border-gray-200 rounded-lg bg-white shadow-sm" style={{ height: '65vh' }}>
          ...
          <div className="px-6 py-3 text-xs text-gray-500">
            ...
            ) : `Showing ${filteredRenewals.length} of ${rows.length} renewals (only items with daysUntilRenewal ≤ 90 are shown)`}
          </div>
        </div>
      </div>
    </div>
