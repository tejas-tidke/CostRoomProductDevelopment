...
export const NotificationProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // Ensure fetchedNotifications is an array before processing
      const notificationsArray = Array.isArray(fetchedNotifications) ? fetchedNotifications : [];
      console.log("%cNOTIFICATION_LOG: Total notifications received:", "color: #6600cc; font-weight: bold;", notificationsArray.length);
      setNotifications(notificationsArray);
      
      // Update unread count
      const unread = notificationsArray.filter((n: AppNotification) => !n.isRead).length;
  ...
  const fetchUnreadCount = async () => {
    try {
      ...
      let count = 0;
      if (typeof response === 'object' && response !== null && 'unreadCount' in response) {
        count = response.unreadCount;
      } else if (typeof response === 'number') {
        count = response;
      } else {
        console.warn('Unexpected unread count response format:', response);
        count = 0;
      }
